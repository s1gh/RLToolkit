...
var Entries = []Entry{
	...
	{Name: "_GoalScored", Category: "scoring", Shape: "goal-enriched", LivePhases: liveOrReplay, Desc: "GoalScored with Scorer/Assister/BallLastTouch resolved + scoringTeam/concedingTeam/isOwnGoal flags + same-frame modifiers (aerial/long/turtle/hatTrick/stolen/etc.). The isOwnGoal flag is heuristic and synchronous (last-touch on conceding team). For verified own-goal detection use _OwnGoal.", Stability: "provisional", Since: "1.1", SubscriptionGroup: "scoring"},
	...
	{Name: "_MatchState", Category: "lifecycle", Shape: "match-state", LivePhases: anyPhase, Desc: "Authoritative gameplay state. matchActive (am I in a match) and phase (lobby/countdown/live/paused/replay/ended/podium/none) on every transition, with previousPhase, phaseDurationSeconds, trigger, and isFreeplay (true when no MatchCreated has fired since the last MatchDestroyed and the roster is at most one player) so subscribers see what changed and why.", Stability: "stable", Since: "2.0"},
